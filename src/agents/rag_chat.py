...
class RAGChatAgent:
    ...
    def __init__(self, top_k: int = 3, category_id: int = 0):
        """
        初始化 RAG Chat Agent
    ...
    def _generate_node(self, state: RAGChatState) -> Dict:
        ...
        messages = self._build_messages(
            state["query"], state["context"], state["use_history"]
        )
    ...
    def _build_messages(
        self, query: str, context: str = None, use_history: bool = True
    ) -> List:
        """构建对话消息"""
        messages = []
    ...
    def add_document(self, question: str, answer: str, metadata: Dict = None):
        ...
        self.vector_store.add_documents(documents=[question], metadatas=[doc_metadata])
    ...
    def batch_add_documents(self, qa_pairs: List[Dict]):
        ...
        for qa in qa_pairs:
            ...
            metadatas.append({"answer": qa.get("answer", ""), **qa.get("metadata", {})})
